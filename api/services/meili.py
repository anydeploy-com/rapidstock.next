...
from config import MEILI_URL, MEILI_KEY, PRODUCT_INDEX, CATEGORY_INDEX, ATTRIBUTE_INDEX
...
class MeilisearchService:
    def __init__(self):
        ...
        try:
            ...
            if hasattr(self.client, "http") and hasattr(self.client.http, "headers") and MEILI_KEY:
    ...
    def add_product(self, product_data):
        try:
            ...
            logger.warning(f"Meilisearch indexing failed for product {product_data.get('id')}: {e}")

    def add_category(self, category_data):
        try:
            self.client.index(CATEGORY_INDEX).add_documents([category_data])
        except Exception as e:
            logger.warning(f"Meilisearch indexing failed for category {category_data.get('id')}: {e}")
